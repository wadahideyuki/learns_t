...
function fncPug2(){
  console.log("\n  styleguide: pugを実行\n");
	return gulp.src([paths.styleguideDir + "/**/*.pug", "!" + paths.styleguideDir + "/**/_*.pug"], {base: paths.styleguideDir})
	.pipe(plumber({
		errorHandler: notify.onError({
			message: "ふぁいる→ <%= error.message %>",
			title: "ぱぐのえらー",
	...
	.pipe(pug({
		basedir: paths.styleguideDir,
		pretty: true
	}))
	.pipe(gulp.dest(paths.styleguideDir));
};
...
function fncScss2(){
  console.log("\n  styleguide: sassを実行\n");
	// .scssファイルを取得
	gulp.src([paths.styleguideDir + '/**/*.scss'])
	// エラーでも止まらないように
	.pipe(plumber({
		// エラーをデスクトップ通知
		errorHandler: notify.onError({
...
gulp.task("watch", function(){
  ...
  gulp.watch(paths.styleguideDir + '/**/*.pug').on('change', fncPug2);
  gulp.watch(paths.scssDir + '/**/*.scss').on('change', fncScss);
  gulp.watch(paths.styleguideDir + '/**/*.scss').on('change', fncScss2);
